...
test('step 1', () => {
	...
	// createComment with empty text
	const empty = createComment('');
	assert.equal(empty.text, '');
	assert.ok(typeof empty.id === 'number');

	// createComment generates unique IDs
	const c1 = createComment('first');
	const c2 = createComment('second');
	assert.notEqual(c1.id, c2.id, 'Each comment should have unique ID');
	...
	// createReply with empty text
	const emptyReply = createReply(1, '');
	assert.equal(emptyReply.text, '');
	assert.equal(emptyReply.parentId, 1);
});
...
test('step 2', () => {
	...
	// likeComment with large number of likes
	const manyLikes = { id: 3, text: 'popular', likes: 1000 };
	const moreLikes = likeComment(manyLikes);
	assert.equal(moreLikes.likes, 1001);
	assert.equal(manyLikes.likes, 1000, 'original must not change');

	// likeComment preserves other properties
	const withExtra = { id: 4, text: 'test', likes: 5, custom: 'value' };
	const liked2 = likeComment(withExtra);
	assert.equal(liked2.custom, 'value', 'Should preserve custom properties');
	assert.equal(liked2.text, 'test', 'Should preserve text');
});
...
test('step 3', () => {
	...
	// deleteComment with non-existent ID
	const res3 = deleteComment(comments, 999);
	assert.deepEqual(res3, comments, 'Should return copy when ID not found');
	assert.notStrictEqual(res3, comments, 'Should be different array');

	// deleteComment with empty array
	const res4 = deleteComment([], 1);
	assert.deepEqual(res4, []);

	// deleteComment: multiple replies in one comment
	const replies = [
		{ id: 10, parentId: 1, text: 'r1', likes: 0 },
		{ id: 11, parentId: 1, text: 'r2', likes: 0 },
		{ id: 12, parentId: 1, text: 'r3', likes: 0 }
	];
	const comment = { id: 1, text: 'c', likes: 0, replies };
	const arr = [comment];

	// Delete first reply
	const res5 = deleteComment(arr, 10);
	const updatedComment = res5[0];
	assert.equal(updatedComment.replies.length, 2);
	assert.ok(!updatedComment.replies.find(r => r.id === 10));
	assert.equal(arr[0].replies.length, 3, 'original should not change');
});
